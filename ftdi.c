...
int ftdi_usb_open_dev(struct ftdi_context *ftdi, libusb_device *dev)
{
    ...
    if ( ( res = libusb_claim_interface(ftdi->usb_dev, ftdi->interface)) < 0)
    {
        ftdi_usb_close_internal (ftdi);
...
int ftdi_eeprom_build(struct ftdi_eeprom *eeprom, unsigned char *output)
{
	...
	output[0x01] = 0x40;
    // Addr 02: Vendor ID
    output[0x02] = eeprom->vendor_id & 0x00FF;
    output[0x03] = eeprom->vendor_id >> 8;

    // Addr 04: Product ID
    output[0x04] = eeprom->product_id & 0x00FF;
...
int ftdi_eeprom_decode(struct ftdi_eeprom *eeprom, unsigned char *buf, int size)
{
    ...
    eeprom->BM_type_chip = buf[0x06] + (buf[0x07] << 8);
    ...
    if (manufacturer_size <= 0) eeprom->manufacturer[0] = 0;

    // Addr 10: Offset of the product string + 0x80, calculated later
    // Addr 11: Length of product string
    product_size = buf[0x11]/2;
    if (product_size <= 0) eeprom->product[0] = 0;

    // Addr 12: Offset of the serial string + 0x80, calculated later
    // Addr 13: Length of serial string
    serial_size = buf[0x13]/2;
    if (serial_size <= 0) eeprom->serial[0] = 0;
